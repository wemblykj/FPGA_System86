...
//  Testbench    : tb_element_extractor_16bit
//  File         : tb_element_extractor_16bit.sv
...
module tb_element_extractor_16bit;
    ...
    initial begin
        ...
        $display("Starting tb_element_extractor_16bit...");
        bus_rdata = 64'hF00D_BEEF_CAFE_1234;
        
        // Test all 16-bit words
        test_addr[0] = 0; // rightmost word = 0x1234
        test_addr[1] = 1; // next word     = 0xCAFE
        test_addr[2] = 2; // next word     = 0xBEEF
        test_addr[3] = 3; // leftmost word = 0xF00D

        for (i = 0; i < 4; i = i + 1) begin
            port_addr = test_addr[i];
            #1;
            expected = bus_rdata[PORT_DATA_WIDTH*port_addr +: PORT_DATA_WIDTH];
            $display("port_addr=%0d data_out=0x%04X (expected=0x%04X)", port_addr, data_out, expected);
        ...
        $display("All 16-bit element_extractor tests passed.");
        #10 $finish;
    end
