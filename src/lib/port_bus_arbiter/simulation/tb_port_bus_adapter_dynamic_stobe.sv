...
module tb_port_bus_adapter_dynamic_strobe;
    ...
    parameter BASE_ADDR_STATIC  = 32'hDEAD_0000;
    ...
    // Localparams
    localparam int BUS_WORD_BYTES   = BUS_DATA_WIDTH / 8;
    localparam int BUS_WORD_SHIFT   = $clog2(BUS_WORD_BYTES);
    ...
    initial begin
        ...
        for (i = 0; i < (1<<CACHE_ADDR_WIDTH); i = i+1) begin
            ...
            b0 = 8'd0 + (i * BUS_WORD_BYTES); // Byte 0 (least significant): 0, 4, 8, ...
            b1 = 8'd1 + (i * BUS_WORD_BYTES); // Byte 1: 1, 5, 9, ...
            b2 = 8'd2 + (i * BUS_WORD_BYTES); // Byte 2: 2, 6, 10, ...
        ...
        if (hit_count == BUS_WORD_BYTES)
            $display("PASS: All addresses in same word hit cache (bitslicing correct)");
        else
            $display("FAIL: Bitslice test failed, %0d/%0d hits", hit_count, BUS_WORD_BYTES);
