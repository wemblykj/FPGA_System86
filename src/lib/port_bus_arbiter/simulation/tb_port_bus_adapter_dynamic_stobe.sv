...
//  Testbench    : tb_port_bus_adapter_dynamic_strobe
//  File         : tb_port_bus_adapter_dynamic_strobe.sv
...
module tb_port_bus_adapter_dynamic_strobe;
    ...
    parameter USE_DYNAMIC_BASE = 1;                 // 0 = static, 1 = dynamic
    parameter USE_STROBE = 1;                       // 0 = RMW, 1 = strobe
    ...
    function automatic bit bus_data_compare_with_strobe;
        int i;
        
        // Arguments:
        //   bus_wdata     - data driven onto the bus (e.g. 32 bits)
        //   bus_wstrb     - strobe bits (e.g. 4 bits for 32-bit bus)
        //   expected_data - reference value for the bus word (same width as bus_wdata)
        //   bus_bytes     - number of bytes in the data word (BUS_DATA_WIDTH/8)
        input logic [31:0] bus_wdata;
        input logic [3:0]  bus_wstrb;
        input logic [31:0] expected_data;
        
        begin
            bus_data_compare_with_strobe = 1'b1; // assume pass
            for (i = 0; i < BUS_WORD_BYTES; i = i + 1) begin
                if (bus_wstrb[i]) begin
                    if (bus_wdata[i*8 +: 8] !== expected_data[i*8 +: 8]) begin
                        bus_data_compare_with_strobe = 1'b0;
                    end
                end
            end
        end
    ...
    initial begin
        ...
        for (i = 0; i < NUM_TESTS; i = i + 1) begin
            ...
            // Assign [known] 'undefined' value to bus_rdata
            ...
            if (!bus_data_compare_with_strobe(bus_wdata, bus_wstrb, test_bus_data[i]))
                $display("FAIL: bus_wdata=0x%08X written to bus_addr=0x%08X with bus_wstrb=0b%04b (expected 0x%08X)", bus_wdata, bus_addr, test_bus_data[i], bus_wstrb);
            else
